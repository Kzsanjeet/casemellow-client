...
console.log("customizeOrderId",customizeOrderId)
  ...
  useEffect(() => {
    const updateStatus = async () => {
      try {
        if (customizeId) {
          const res = await fetch(`${process.env.NEXT_PUBLIC_LOCAL_PORT}/order/customize/update-status/${customizeId}`, {
            method: 'PATCH',
        ...
        if (customizeCodId) {
          const res = await fetch(`${process.env.NEXT_PUBLIC_LOCAL_PORT}/order/customize/cod/add-order`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ customizeOrderId: customizeCodId }),
          });
          const data = await res.json();
